...
然而，我们仍然必须明确指定被 pin 的引用的类型：否则 Rust 仍然不知道如何将它们解释为动态Trait对象，而这正是我们在 Vec 中所需要的。因此我们在定义每个 future 的时候使用 `pin!`，并将 futures 定义为一个包含对动态 future 类型的被固定的可变引用的 Vec。(译者注：整段话仍需准更确翻译)。如示例 17-19 所示：通过'pin!'宏来直接使用'pin'以避免不必要的堆分配。
...
这是一个基础的权衡取舍：要么我们可以使用 `join_all` 处理动态数量的 future，只要它们都有相同的类型（译者注：需要用其他类型封装，再使用）；要么我们可以使用 `join` 函数或者 `join!` 宏来处理固定数量的 future，哪怕它们有着不同的类型(译者注：可以直接使用)。不过这与 Rust 处理任何其它类型是一样的。Future 并不特殊，即便我们采用了一些友好的语法来处理它们，而这其实是好事。
...
回忆一下[第一个异步程序][async-program]中提到，在每一个 await点，如果被 await 的 future 还没有就绪，Rust 会给运行时一个机会来暂停该任务并切换到另一个任务。反过来也是正确的：Rust *只会* 在一个 await点暂停异步代码块并将控制权交还给运行时。await点之间的一切都是同步。

这意味着如果你在异步代码块中做了一堆工作而没有一个 await点，则那个 future 会阻塞其它任何 future 继续进行。有时你可能会听说这称为一个 future *starving*(饿死) 其它 future。在一些情况中，这可能不是什么大问题。不过，如果你在进行某种昂贵的设置或者长时间运行的任务，亦或有一个 future 会无限持续运行某些特定任务的话，你会需要思考在何时何地将控制权交还运行时。
...
与上一个示例一样，`race` 仍然在 `a` 完成后就立刻结束了。两个 future 之间没有交替运行。`a` future 一直进行其工作直到 `trpl::sleep` 调用被 await，然后 `b` future 一直进行其工作直到它自己的 `trpl::sleep` 调用被 await，再然后 `a` future 才完成。为了使两个 future 在各自缓慢任务之间都能有所进展，我们需要 await点才能将控制权交还给运行时。这意味着我们需要一些可以 await 的东西！
...
这意味着，即使对于受计算限制的任务，异步也可能很有用，具体取决于你的程序还在做什么，因为它为构建程序不同部分之间的关系提供了一个有用的工具。这是一种协作式多任务处理形式(cooperative multitasking)，其中每个future都有权决定何时通过 await 点移交控制权。因此，每个future也有责任避免长时间阻塞。在一些基于 Rust 的嵌入式操作系统中，这是唯一的多任务处理方式！

当然，在实际代码中，你通常不会在每一行上都交替使用函数调用和 await 点。虽然以这种方式让出控制权相对成本较低，但也不是没有代价的。在许多情况下，试图分解受计算限制的任务可能会使其显著变慢，所以有时为了整体性能，最好让一个操作短暂阻塞。应该通过测试你的代码以了解其实际性能瓶颈在哪里。不过，如果看到很多本应并发执行的工作却在串行执行，那么记住这种潜在的动态是很重要的。
...
这满足了我们对类型的要求。现在让我们思考下所需的 *行为*：我们需要传递进来的 future 在持续时间内相互竞争。我们可以使用trpl::sleep从持续时间创建一个定时器future，并使用trpl::race将该定时器与调用者传入的future一起运行。

我们还知道 `race` 是不公平的，并按照参数传递的顺序轮询。因此，我们首先传递 `future_to_try` 给 `race`， 这样即使max_time非常短，它也有机会完成。如果 `future_to_try` 首先完成，`race` 会返回 `Left` 和 `future` 的输出。如果 `timer` 首先完成，`race` 会返回 `Right` 和定时器的输出 `()`。
...
- 我们在 `Vec` 上使用了 `join_all` 来等待某个组中的所有 future 完成。相反该如何使用 `Vec` 来依次处理一个序列的 future 呢？这么做有哪些权衡取舍呢？
- 仔细观察 `futures` crate 中的 `futures::stream::FuturesUnordered` 类型。使用它与使用 `Vec` 又有什么区别呢？（不用担心它来自与 crate 的 `stream` 部分的事实；它与任何future集合都能很好地配合使用。）
